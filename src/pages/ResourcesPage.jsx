  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="relative mb-20 p-10 bg-gray-50 rounded-lg min-h-[300px] flex items-center justify-center">
        {/* Central Title */}
        <h2 className="text-3xl font-bold bg-white p-4 rounded-lg shadow z-10">
          Resource Sections
        </h2>

        {/* DESKTOP (lg): Absolute, fancy positioning */}
        ...
        {/* MOBILE/TABLET: Horizontal scroll bubbles at bottom */}
        <div className="lg:hidden absolute left-0 right-0 bottom-4 flex gap-4 overflow-x-auto px-4 scrollbar-hide z-10">
          {resources.map((res) => (
            <button
              key={res.id}
              onClick={() => scrollToSection(res.id)}
              className={`min-w-max p-3 rounded-lg shadow text-white font-semibold cursor-pointer hover:scale-105 transition-transform ${res.bubbleColor}`}
            >
              {res.title}
            </button>
          ))}
...
function getBubbleStyle(index) {
  const styles = [
    { top: "15%", left: "32%" }, // e.g. Digital wellness
    { top: "18%", left: "60%" },
    { top: "60%", left: "20%" },
    { bottom: "25%", right: "20%" },
    { bottom: "10%", left: "40%" },
