...
function ProgramSection({ program }) {
  ...
  switch (program.id) {
    case "mentor-program":
      ...
      break;
    case "well-being-representatives":
  ...
  const InfoBlock = ({ title, content, className = "" }) => (
    <div
      className={`p-5 rounded-lg border shadow-sm ${infoBlockColor} ${className}`}
    >
      ...
      {typeof content === "string" && content.length > 0 && (
        <p className="text-md md:text-lg lg:text-lg text-gray-700">{content}</p>
      )}
    </div>
  );
      ...
      <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <InfoBlock
            ...
            content={program.roles}
          />

          <InfoBlock
            className="md:col-span-2"
            title={program.benefitsTitle}
            content={program.benefits}
          />

          <div className="md:col-span-2 flex items-center justify-center p-4">
            <h3
              className={`text-xl md:text-2xl lg:text-2xl font-bold ${bubbleTextColor} ${bubbleColor} py-3 px-8 rounded-full shadow`}
            >
              {program.title}
            </h3>
          </div>
        ...
        <div className="flex justify-center mb-8">
          <h3
            className={`text-lg md:text-xl lg:text-xl font-bold text-gray-700 ${bubbleColor} py-3 px-8 rounded-full shadow`}
          >
            {program.actionTitle}
          </h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-2 items-center">
          ...
          <div className="rounded-lg h-60 flex items-center justify-center">
            <img
